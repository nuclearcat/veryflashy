...
|:--------|:-----------:|:-------------:|:------------:|
| ASolid | IS918 family, tested with 18002S firmware | ✓ | Model heuristic, firmware, programmed USB identity/capacity, CE/LUN counts, conditional capacity-gap estimate and MPTool metadata; [protocol and support limits](docs/asolid.md) |
...
The recorded CE/LUN counts describe configured topology. The capacity-gap
estimate uses the NAND database's per-die density when unambiguous; it does
not measure reserve allocation or establish the number of independent channels.
